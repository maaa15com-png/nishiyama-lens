...
export default async function NearbySpots({ lensId }: { lensId: string }) {
  ...
  return <section aria-labelledby="nearby-title" className="mt-12 border-t border-[#d9ddd3] pt-9">
    ...
    <h2 id="nearby-title" className="mt-3 text-2xl font-medium leading-relaxed">このLENSなら、次は鯖江のまちへ</h2>
    <p className="mt-4 text-sm leading-8 text-[#53665a]">選んだ楽しみ方に合う立ち寄り先を、鯖江でも。見学・撮影条件は各施設の公式案内をご確認ください。</p>
    <ul className="mt-6 grid gap-5 sm:grid-cols-2">{spots.map((spot) => <li key={spot.id} className="min-w-0 rounded-3xl border border-[#e0e3d9] bg-[#fffdf8] p-6">
      ...
      {spot.externalUrl && <a href={spot.externalUrl} target="_blank" rel="noopener noreferrer" aria-label={`${spot.name}の施設の案内を見る（別タブ）`} className="mt-5 inline-flex min-h-12 items-center rounded-2xl border border-[#b9c3b8] px-5 py-3 text-sm font-semibold leading-7 underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-offset-4">施設の案内を見る（別タブ）</a>}
    </li>)}</ul>
  </section>;
}
